...
Point lights are computed using a shader invocation for each voxel. The inputs to this shader are light position, initial light intensity, decay power, and distance power. The light position is used to compute the vector along which the light will travel, as well as serve as the origin point for that ray. There is code to handle the cases in which the light is in inside the voxel under consideration or if the light position falls outside the volume.
