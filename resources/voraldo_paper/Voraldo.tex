...
This process is repeated until the ray reaches or passes the location of the current voxel. The value of distance power is then used to compute a scale factor that approximates the inverse square law, at the default value of 2. It can be set to zero to have the scale factor go to one, or raised to a higher power to decay more in a shorter distance. The final result of the computation is then added to the existing value in voxel in the lighting buffer associated with the shader invocation.
